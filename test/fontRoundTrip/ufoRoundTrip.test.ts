...
describe('UFO import → export → reimport round-trip', () => {
  ...
  it('rewrites an untouched project byte for byte, glyphs and font level alike', async () => {
    ...
    // One file is expected to differ, and saying so here is the point: the list
    // is the contract. Anything else that starts differing is a bug.
    //
    // lib.plist gains Kumiko's own com.kumiko.fontEditor.* keys. That is what a
    // UFO lib is for, and the reverse-DNS prefix is the spec's mechanism.
    const KNOWN_DIFFERENT = ['lib.plist']
...
`
  ...
  it('keeps kerning and feature sources in the exported UFO', async () => {
    ...
    // Nobody edited the features, so the file the source shipped is written back
    // as it was. This used to assert the opposite — that the hand-written header
    // was gone and a "Generated by Kumiko Font Editor." banner had replaced it —
    // which made renaming the family rewrite the author's feature file.
    expect(exportedFeatureText).toBe(originalFeatureText)
    expect(paths.some((p) => p.endsWith('.ufo/glyphs/contents.plist'))).toBe(
      true
    )
  })

  // The other half of the contract, and the one that must never break: once the
  // model no longer matches the file that produced it, the file is stale and
  // writing it back would silently discard the edit. Editing a rule in place
  // inside an imported lookup is the hard case — the lookup keeps its id and its
  // 'imported' origin, so no edit flag would catch it.
  it('generates features once an imported rule has been edited', async () => {
    const entries = await readUfoEntries()
    const imported = await importWorkspace(entries, UFO_NAME)
    const features = imported.fontData.openTypeFeatures!
    const edited = {
      ...features,
      lookups: features.lookups.map((lookup, index) =>
        index === 0
          ? {
              ...lookup,
              rules: lookup.rules.map((rule, ruleIndex) =>
                ruleIndex === 0
                  ? { ...rule, firstValue: { xAdvance: -999 } }
                  : rule
              ),
            }
          : lookup
    ...
    const blob = await exportCanonicalFontDataAsUfoZip({
      fontData: { ...imported.fontData, openTypeFeatures: edited },
      projectId: 'canonical-ufo-features-edited',
      projectTitle: 'OpenSourceFont',
      projectSourceData: imported.projectSourceData,
      projectSourceFormat: imported.projectSourceFormat,
    })
    ...
    const path = Object.keys(files).find((name) =>
      name.endsWith('.ufo/features.fea')
    )
    expect(path).toBeDefined()
    const text = strFromU8(files[path!])

    expect(text).toContain('pos A V -999;')
    expect(text).not.toContain('pos A V -80;')
  })
