...
expectedFeature =
  currentPeerMedian + referenceResidual * confidence * referenceStyleScale
```

也就是說，參考字體只提供「這個字相對同儕的自然偏移」。目前字體自己的整體風格仍由 `currentPeerMedian` 決定；`referenceStyleScale` 則從目前字體數千個共同字 robust fit，描述這套字沿用參考字逐字差異的幅度。一般方體可能接近 1，窄體或高度風格化字體可以自動收縮到接近 0，不用靠辨識字體名稱或預設「窄體模式」。

風格縮放只套用形狀類 residual。每個 feature 至少要有 100 個有效共同字，使用三輪 Huber IRLS 擬合通過原點的 slope，降低少量畫壞字與參考特例的影響；樣本不足時維持 1。系統也保存映射後 residual 的 robust error，作為 reference z-score 的額外尺度下限。這表示即使平均縮放可以對齊，只要兩套字在逐字決策上仍常不一致，reference 尺就會自動降低自信。
...
**形狀與放置適用不同的信任規則**（見「Radar 怎麼產生建議」）。形狀 residual 經目前字體的 style scale 與映射誤差校正後，可直接平移期待值；放置決策（部件靠邊還是置中、留白分配）仍帶有各家排印慣例，單一參考字體不足以定罪——例如 Noto 的「片」右邊距留到 230/1000，兩套優質 TC 字體都只留 56–93。
...
proportion 另含**投影間隙** `gap:x` / `gap:y`：把輪廓線段投影到軸向 bin，取字面內最寬「無墨空帶」相對字面的比例（封閉輪廓在某列有墨，邊界必穿過該列，故無線段跨越＝真空帶）。左右／上下部件被拉開時，外框、邊距、重心可能都不動，只有這個特徵會動——它是整字聚合特徵抓不到的失效模式。州、三這類天生大間隙的字由 style-adapted reference residual 吸收。

proportion 也含**語意部件介面間距** `part-gap:x` / `part-gap:y`。`partSpacingMetrics.ts` 從 GlyphWiki 直接組成資料辨識兩部件的左右／上下配置，以語意分界將攤平輪廓分到兩側，再沿兩部件共同覆蓋的 64 條 scanline 計算相向邊緣距離，取中位數正規化到字面。它跟全域投影空帶的差別是：少量橫筆跨過部件介面時，其他 scanline 仍可量到真正間距。

第一版只處理兩個直接部件，且只對可信的正向空隙提示「太開」。攤平商業字體沒有 component lineage，contour-centroid 分件對互相穿插的部件仍可能把跨界筆畫分錯側；負間距／擁擠提示要等 raster mask 分件後才啟用。

部件介面間距使用兩層基準：

- **批次檢查**：優先和相同左部件／上部件的字比較，樣本不足退回同軸結構。因單一參考字體沒有這項 residual，peer z 另做保守收縮，避免成熟字體的天然部件差異變成誤報。
- **編輯即時檢查**：母體快照保留同一 glyph 的 `partSpacing`，目前字和編輯前快照直接比較。這條 `basis: 'baseline'` 的尺不必猜「語」天生應留多少空間，能對小幅部件移動保持敏感。

### Raster mask 分件原型

`rasterPartSpacingMetrics.ts` 提供 128×128 occupancy mask 原型。它把封閉 contour 填入 mask，以 GlyphWiki 語意分界切成兩側，逐列／逐欄量最近墨點，同時計算兩側墨群質心分離度。跨界筆畫在局部只會成為 gap 0，不會像整條 contour 分錯部件時產生巨大負間距。

校正 harness 可用以下環境變數切換或同時比較：

```bash
QUALITY_PART_SPACING_METHOD=raster QUALITY_CALIBRATION=1 pnpm vitest run test/qualityCheck/radarCalibration.test.ts
QUALITY_COMPARE_PART_SPACING=1 QUALITY_CALIBRATION=1 pnpm vitest run test/qualityCheck/radarCalibration.test.ts
```

目前比較結果：

| 字體      | contour suspect | raster suspect | contour 負間距 |  raster 接觸字 |
| --------- | --------------: | -------------: | -------------: | -------------: |
| LINE Seed |           1.66% |          1.65% |   365 / 10,540 | 4,691 / 10,540 |
| 金萱      |           2.40% |          2.33% |   864 / 10,963 | 4,087 / 10,963 |
| Road Font |           3.21% |          3.43% |    567 / 6,758 |  3,679 / 6,758 |
| 中明體    |           3.82% |          3.88% |   413 / 14,404 | 4,444 / 14,404 |

raster 對所有有語意配置的字都能產生量測，也完全消除負 gap；但 31–54% 的字有跨界筆畫，純 gap 中位數常為 0。加入墨群分離度後，±1% UPM 位移在五個校正 face 都達到既定 80% suspect／正確理由門檻，中明體較弱的一個 face 為 166/200 suspect、178/200 正確理由，仍低於 contour 的 200/200。

因此 production 目前仍使用 contour gap；raster 保留為第二訊號原型。下一步較合理的方向不是直接替換，而是：即時編輯時按需用 separation 捕捉互相拉遠、用 touching ratio 的增量捕捉互相擠近；批次檢查則要先做帶完整部件框 seed 的 watershed／distance assignment，再校正天然跨界筆畫。
...
- **形狀類**（`face:*`、`ink:toFace`）：reference residual 先乘目前字體學到的 style scale，並以映射誤差擴張不確定範圍，再取代同儕中位（`basis: 'reference'`）。這讓「冑本來就窄」仍能得到結構校正，但 Road Font／中明體不必完整沿用 Noto 的逐字寬度振幅。
...
- `RadarReason.basis` 會標記建議來自 `peers`、`reference` 或 `baseline`，UI 文案會顯示「複雜度相近的字」、「參考結構校正值」或「編輯前的字形」。
...
- 左右部件相向邊緣的間距比編輯前偏開：將左右部件互相靠攏。

## 用優質字體校正

演算法的門檻與信任規則不是拍腦袋定的，而是用已知高品質的商業字體回歸出來的。校正 harness 在 `test/qualityCheck/radarCalibration.test.ts`，預設優先讀 `test_glyphs/good_quality_font/`，不存在時退回 `test_glyphs/`；也可用 `QUALITY_CALIBRATION_FONT_DIR` 指定目錄。OTF/TTF 由 opentype.js 直接讀取；TTC 則由 `calibrationFontFaces.ts` 使用專案 Pyodide 內附的 fontTools wheel 離線拆出每個 face，再走同一條取樣管線。所有 face 都會轉成與編輯器相同的 `GlyphGeometrySample`，走真正的 `computeRadarFromSamples`：
...
- **誤報率**：優質字體被列為可疑的比例要低。加入 reference style adaptation 後，金萱為 2.40%、LINE Seed 為 1.66%、Road Font 為 3.21%，中明體兩個 TTC face 都是 3.82%。Road Font 的 `face:widthRatio` style scale 自動學到 0.422（固定 residual 版本為 5.40% suspect），中明體為 0.312；金萱與 LINE Seed 分別為 0.805、0.687。中明體兩個 face 的漢字輪廓與指標相同，差異主要不在漢字區。
- **召回率**：除了整體右移 6% UPM、字面縮小 15%、直接竄改 gap 到字面 30% 的舊測試，現在會依 GlyphWiki 語意分界真的把兩側輪廓各向外移 1% UPM，再完整重算 bounds、ink、side 與 part spacing。金萱 199/200、LINE Seed 200/200、Road Font 200/200 被列為 suspect；三套分別有 199/200、200/200、199/200 給出正確的 `part-gap` baseline 理由。
...
- `radar.partSpacingBaselineByGlyphId` 保存母體快照中的同字部件間距；`GlyphInsightProvider` 在切入該字時凍結這個值，後續母體 refresh 不覆寫，直到切換字形才換尺。小幅移動因此不必等到偏離全字體同儕才提示。
...
- 可進 worker 的純層包括 `polygonGeometry`、`glyphInk`、`partSpacingMetrics`、`rasterPartSpacingMetrics`、`radarFeatureCollection`、`radarStatistics`、`radarReferenceModel`、`referenceStyleModel`、`hanClassification`、`structureMetrics`、`structureRuler`、`glyphSampling`、`qualityRadar`、`populationAnalysis`，以及只用 `fetch` 載靜態資料的 `semanticStructure` / `radarReferenceData`。`openTypeSampling` 是給校正 harness、raster/contour A/B 與參考資料建置用的 opentype.js 轉接層，同樣是純函數。
