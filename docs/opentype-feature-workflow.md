...
- GSUB contextual parser
  ...
  - `GSUB type 8 format 1` reverse chaining single substitution 會讀取 backtrack / lookahead coverage，並展開成 editable reverse-chaining single substitution rules。
...
- GPOS contextual parser
  - `GPOS type 7 format 1` 會讀取 glyph-sequence context positioning rules 與 positioning lookup records。
  - `GPOS type 8 format 1` 會讀取 glyph-sequence chaining context positioning rules 與 positioning lookup records。
...
- lookup 內的 `subtable;` break hint 會被接受，重建時由 compiler 重新決定 subtable 切分
- enumerated pair positioning，例如 `enum pos @Left @Right -80;`，會展開成 glyph-pair rules
...
2. FeatureVariations / extension lookup 的呈現與 preservation policy。
   - ExtensionSubst / ExtensionPos 會攤平成可編輯 rules，並在 lookup meta、diagnostics、export warning 中標示 rebuild 只保證等價行為，不保證保留 extension wrapper shape。
   - FeatureVariations 會被標成 table-level compiled-only source data；即使同一張表的 lookups 都可編輯，source section 仍會走 `preserve-if-unchanged`，export impact 會提示 rebuild 前需檢查 preservation。
